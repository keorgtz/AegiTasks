...
Focus permite seleccionar pendientes no completados, no archivados, propios o sin responsable de los proyectos activos del espacio actual. El servidor vuelve a comprobar esas condiciones al iniciar, por si otro miembro cambió un pendiente mientras se elegía. La bandeja aplica por defecto el mismo filtro de responsable para cualquier rol, incluido Admin. El selector permite consultar otros responsables y las métricas respetan ese filtro; esto no modifica los permisos de acceso al espacio. El cambio no necesita migraciones de base de datos.
