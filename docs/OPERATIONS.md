...
Crear, importar y abrir notas lleva a una página completa (`#notes/new` o `#notes/<id>`), que ocupa toda la ventana de la app en escritorio y móvil. Sidebar y navegación global se ocultan hasta pulsar «Volver a notas». El encabezado mantiene título editable, estado de guardado, guardar, selector de vista y el botón para ocultar/mostrar herramientas. La toolbar usa controles compactos con áreas táctiles de 44 px cuando corresponde. Ocultarla amplía el canvas sin modificar el texto ni la vista seleccionada. Los paneles de edición/preview se desplazan por separado; la página exterior permanece ajustada a la ventana.

La vista inicial es dividida a partir de 1024 px y solo edición en pantallas menores. «Opciones» abre las propiedades, plantillas y exportaciones; Escape cierra ese panel. Ctrl/⌘ + S guarda tanto desde el título como desde el contenido. Navegar con cambios pendientes pide confirmación; la PWA pospone la actualización mientras hay un borrador, guardado o exportación en curso.
...
«Diagrama Mermaid» inserta uno de 39 ejemplos editables organizados en siete categorías. Cubre las familias nativas incluidas en Mermaid 11.17.2: flujo, carriles, secuencia, clases, estados, ER, recorrido, cronograma, timeline, Kanban, cuadrantes, requisitos, Git, bloques, infraestructura, eventos, paquetes, árbol, mapa mental, pie/XY, Sankey, radar, treemap, Venn, Ishikawa, Wardley, Cynefin, cinco vistas C4 y cuatro notaciones de gramática (primitivas, EBNF, ABNF y PEG). La plantilla de flujo avanzado muestra grupos, estilos y descripciones accesibles. No incluye extensiones externas como ZenUML ni tipos introducidos después de esa versión. También puedes escribir bloques con la etiqueta `mermaid`, según la [sintaxis oficial de Mermaid](https://mermaid.js.org/intro/syntax-reference.html). Los diagramas se procesan en el navegador con modo estricto, HTML desactivado y SVG saneado; los errores muestran el código original para corregirlo. Cada bloque admite hasta 20 000 caracteres y 300 conexiones. No se admiten directivas/frontmatter de configuración, imágenes externas ni CSS que cargue recursos. Las exportaciones Markdown/ZIP conservan el código; HTML/PDF incluyen los diagramas renderizados. HTML no necesita Mermaid ni conexión para mostrar los SVG y fuentes embebidas. Si un diagrama tiene errores, la exportación conserva el aviso y su código.
