...
public static class FocusEndpoints
{
    public static void MapFocus(this WebApplication app)
    {
        ...
        group.MapPut("/profile", async (ProfileInput input, AppDb db, ClaimsPrincipal user) => {
            ...
            if (input.Theme is not ("aurora" or "waves" or "terminal" or "fireflies" or "breeze" or "constellation" or "codeRain" or "geometry")) throw new InputError("Tema no válido.");
            if (input.AccentColor != null && !System.Text.RegularExpressions.Regex.IsMatch(input.AccentColor, "\\A#[0-9a-fA-F]{6}\\z")) throw new InputError("Selecciona un color hexadecimal válido.");
            if (input.ParticleShape != null && input.ParticleShape is not ("mixed" or "circles" or "triangles" or "squares" or "hexagons")) throw new InputError("Forma no válida.");
            ...
            if (input.AccentColor != null) p.AccentColor = input.AccentColor.ToUpperInvariant();
            if (input.ParticleShape != null) p.ParticleShape = input.ParticleShape;
    ...
    public record ProfileInput(int FocusMinutes, int ShortBreakMinutes, int LongBreakMinutes, int Cycles, string Theme, bool Animated, bool Sound, string? AccentColor = null, string? ParticleShape = null);
